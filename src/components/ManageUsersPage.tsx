...
import { PlusIcon, EditIcon, DeleteIcon, IdCardIcon, KeyIcon, LockClosedIcon, LockOpenIcon, FolderOpenIcon, ArrowLeftIcon } from './Icons.tsx';
...
const UserFormModal: React.FC<{
    ...
    const allowedRoles = useMemo(() => {
        if (isSuperAdmin) {
            // Super Admin can create Principals primarily, but also other roles if navigating deep
    ...
    const [formData, setFormData] = useState<Partial<User>>({
        ...
        college_code: user?.college_code || (currentUser.role !== Role.SUPER_ADMIN ? currentUser.college_code : ''),
    });
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="role" value={formData.role} onChange={handleInputChange} className={inputClasses} disabled={isEditMode && !isSuperAdmin}>
                         ...
                         {currentUser.role === Role.SUPER_ADMIN && (
                            <div className="md:col-span-2">
                                ...
                                />
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Assign a unique code. All data for this college will be tied to this code.</p>
                            </div>
                        )}
                        <div>
...
const AccessConfirmationModal: React.FC<{
    ...
    const subText = isRevoking && user.role === Role.PRINCIPAL 
        ? `Blocking this Principal will also suspend access for ALL users (HODs, Faculty, Staff, Students) under college code ${user.college_code}.` 
        : `This action will affect ${user.name}.`;
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in" onClick={onClose} aria-modal="true" role="dialog">
            <div 
                className="bg-white dark:bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl w-full max-w-md m-4 animate-scale-in"
...
const ManageUsersPage: React.FC<{ user: User | null }> = ({ user: authenticatedUser }) => {
    ...
    const [selectedPrincipal, setSelectedPrincipal] = useState<User | null>(null);
    const [error, setError] = useState<string | null>(null);
    
    const fetchUsers = async () => {
        if (authenticatedUser) {
            try {
                const users = await getUsers(authenticatedUser);
                setAllUsers(users);
    ...
    const { principals, hodsAndFaculty, staff, students } = useMemo(() => {
        // SUPER_ADMIN filters logic:
        // If super admin has NOT selected a principal, we only show principals list.
        // If super admin HAS selected a principal, we show data filtered by that principal's college_code.
        
        const usersToFilter = allUsers.filter(u => u.role !== Role.SUPER_ADMIN);
        
        // If Super Admin and viewing details of a specific college
        if (isSuperAdmin && selectedPrincipal) {
             const collegeCode = selectedPrincipal.college_code;
             return {
                principals: [], // Don't show principals list when viewing details
                hodsAndFaculty: usersToFilter.filter(u => (u.role === Role.HOD || u.role === Role.FACULTY) && u.college_code === collegeCode),
                staff: usersToFilter.filter(u => u.role === Role.STAFF && u.college_code === collegeCode),
                students: usersToFilter.filter(u => u.role === Role.STUDENT && u.college_code === collegeCode)
             };
        }
        
        // If Super Admin and viewing main list
        if (isSuperAdmin) {
             return {
                principals: usersToFilter.filter(u => u.role === Role.PRINCIPAL),
                hodsAndFaculty: [],
                staff: [],
                students: []
             };
        ...
        // Standard view for Principals/HODs
        return {
            principals: usersToFilter.filter(u => u.role === Role.PRINCIPAL),
            hodsAndFaculty: usersToFilter.filter(u => u.role === Role.HOD || u.role === Role.FACULTY),
            staff: usersToFilter.filter(u => u.role === Role.STAFF),
            students: usersToFilter.filter(u => u.role === Role.STUDENT)
        };
    }, [allUsers, isSuperAdmin, selectedPrincipal]);
    ...
    // Visibility logic
    const canSeePrincipals = authenticatedUser?.role === Role.SUPER_ADMIN || authenticatedUser?.role === Role.PRINCIPAL; // Principal sees themselves
    ...
    const handleConfirmToggleAccess = async () => {
        ...
        try {
            await deleteUser(confirmAccessState.user.id, authenticatedUser);
            await fetchUsers();
            setConfirmAccessState({ isOpen: false, user: null, action: 'revoke' });
        } catch (err) {
            setError(`Failed to update access: ${(err as Error).message}`);
        }
    };
    ...
    const handleDeleteUser = async (userToDelete: User) => {
        ...
        if (window.confirm(confirmMessage)) {
            try {
                await deleteUser(userToDelete.id, authenticatedUser, isHardDelete);
                fetchUsers();
            } catch (err) {
    ...
    const handleGenerateIdCard = async (userToGenerate: User) => {
        try {
            ...
            setError(`Could not generate ID card. See console for details.`);
        }
    };
    ...
    const handleSaveUser = async (userToSave: User) => {
        if (!authenticatedUser) return;
        setError(null);
        // If super admin is adding a user while inside a college view, force the college code
        if (isSuperAdmin && selectedPrincipal && userToSave.role !== Role.PRINCIPAL) {
            userToSave.college_code = selectedPrincipal.college_code;
        }
        ...
        try {
            ...
            setError(`Failed to save user: ${(err as Error).message}`);
        }
    };
    ...
    const handleManageCollege = (principal: User) => {
        setSelectedPrincipal(principal);
    };

    const handleBackToPrincipals = () => {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            {isSuperAdmin && selectedPrincipal && (
                <div className="mb-6 flex items-center gap-4">
                    <button onClick={handleBackToPrincipals} className="flex items-center gap-2 text-primary-500 hover:text-primary-400 transition-colors">
                        <ArrowLeftIcon className="w-5 h-5" />
                        <span className="font-semibold">Back to Colleges</span>
                    </button>
                    <div className="h-6 w-px bg-slate-700"></div>
                    <span className="text-slate-400">Managing: <span className="text-white font-bold">{selectedPrincipal.name}</span> (College Code: {selectedPrincipal.college_code})</span>
                </div>
            )}

            <div className="space-y-8">
                {/* Super Admin View: Show Principals List Only if no college selected */}
                {canSeePrincipals && (!isSuperAdmin || !selectedPrincipal) && (
                    <UserTable 
                        title={isSuperAdmin ? "Registered Colleges (Admins)" : "College Admins"} 
                        users={principals} 
                        ...
                        onViewData={isSuperAdmin ? handleManageCollege : undefined}
                    />
                )}

                {/* Detailed Views: Show if not super admin OR if super admin has selected a principal */}
                {(!isSuperAdmin || selectedPrincipal) && (
                    <>
                        {canSeeAcademics && (
                            <UserTable 
...
}> = ({ title, users, canManage, onAdd, onEdit, onDelete, onGenerateIdCard, onChangePassword, onToggleAccess, onViewData, authenticatedUser }) => {
    const isSuperAdmin = authenticatedUser?.role === Role.SUPER_ADMIN;
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <div className="ml-4">
                                            ...
                                            <div className="mt-1 flex items-center gap-2">
                                                ...
                                                {user.college_code && isSuperAdmin && <span className="text-xs text-slate-500 font-mono">[{user.college_code}]</span>}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                 ...
                                 <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                                    {user.access_revoked ? (
                                        <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300`}>
                                            {isSuperAdmin && user.role === Role.PRINCIPAL ? "Blocked" : "Access Revoked"}
                                        </span>
                                    ) : (
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    {canManage ? (
                                        <div className="flex justify-end gap-1">
                                            {onViewData && user.role === Role.PRINCIPAL && !user.access_revoked && (
                                                <button onClick={() => onViewData(user)} title="View College Data" className="text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors mr-2">
